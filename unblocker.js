// Define the function to run the killer
function runCode() {
    ...
    toggleButton.addEventListener("click", function() {
        ...
        if (this.textContent === "OFF") {
            ...
            this.textContent = "ON";
            const o = 32 * 99;
            // Setting expiration date to a distant future date (e.g., year 2100)
            const t = new Date("2100-01-01").toUTCString();
...
// Execute the runCode function in all tabs
chrome.tabs.query({}, function(tabs) {
    tabs.forEach(tab => {
        chrome.tabs.executeScript(tab.id, { code: '(' + runCode + ')()' });
    });
});
